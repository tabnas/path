...
# rs/ via path dependencies on the sibling checkouts.
...
# It rewrites go/path.go ONLY: ts/package.json, ts/src/path.ts and the Rust
# sites are the release orchestrator's job (admin/publish.sh), and the
# version tests in every runtime fail the build if they drift.
publish-go: test-go
...
# `--all-targets` excludes doctests, and the README's examples are
...
# not accept a path dependency, so tabnas-path is not published. Only the
...
reset:
	...
	cd rs && cargo clean && cargo build --all-targets && cargo test --all-targets && cargo test --doc
