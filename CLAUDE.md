...
path-array allocation difference, the plugin contract, build & test
commands, and how to treat untrusted input.
...
**This is a core component.** At runtime its only fleet dependency
is the engine: `@tabnas/parser`, `github.com/tabnas/parser/go`, or the
`tabnas` crate. Its C library runs on the engine too, with the grammar
supplied by the caller as a GrammarSpec. It must never depend on a
grammar such as jsonic (admin ADR-22).
