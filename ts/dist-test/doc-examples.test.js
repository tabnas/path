...
function collectMarkdown() {
    ...
    const walk = (dir) => {
        ...
        for (const e of fs.readdirSync(dir)) {
            if (e === 'node_modules' || e.startsWith('dist') || e === '.git')
                continue;
...
function importsToRequire(code) {
    return code
        ...
        .replace(/^\s*import\s+(\w+)\s+from\s+(['"][^'"]+['"]).*$/gm, 'const $1 = require($2)');
}
...
function rewriteAssertions(code) {
    ...
    const out = code.split('\n').map((line) => {
        ...
        if (expected === '')
            return line; // `// =>` with no value: leave as comment
    ...
    return function patchedRequire(spec) {
        try {
            ...
            if (e && e.code === 'MODULE_NOT_FOUND') {
                ...
                if (m)
                    return require(path.join(TABNAS, m[1], 'ts') + (m[2] || ''));
            }
            throw e;
        }
    };
...
describe('doc-examples', () => {
    ...
    for (const file of files) {
        ...
        blocks.forEach((b, bi) => {
            ...
            if (!ARROW.test(joined))
                return; // no assertions -> skip
