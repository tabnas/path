...
func runSpecFile(t *testing.T, path string) {
	for _, row := range loadSpec(t, path) {
		t.Run(specLabel(row.input), func(t *testing.T) {
			...
			// plays that part in both runtimes, and addPathCapture annotates each
			// node with the path the plugin tracked, which is what a fixture pins.
			j := newParser()
			addPathCapture(j)
			got, err := j.ParseMeta(row.input, opts)
